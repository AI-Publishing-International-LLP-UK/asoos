...
/**
 * AI Service Provider Configurations for OAuth2
 */
const AI_PROVIDERS = {
  anthropic: {
    tokenEndpoint: 'https://auth.anthropic.com/oauth2/token',
    authEndpoint: 'https://auth.anthropic.com/oauth2/authorize',
    revokeEndpoint: 'https://auth.anthropic.com/oauth2/revoke',
    scopes: ['messages:write', 'messages:read', 'models:read', 'usage:read'],
    defaultRedirectUri: 'https://sallyport.2100.cool/oauth/claude/callback'
  },
  openai: {
    tokenEndpoint: 'https://auth.openai.com/oauth2/token',
    authEndpoint: 'https://auth.openai.com/oauth2/authorize', 
    revokeEndpoint: 'https://auth.openai.com/oauth2/revoke',
    scopes: ['api.read', 'api.write', 'models.read', 'usage.read'],
    defaultRedirectUri: 'https://sallyport.2100.cool/oauth/openai/callback'
  },
  hume: {
    tokenEndpoint: 'https://auth.hume.ai/oauth2/token',
    authEndpoint: 'https://auth.hume.ai/oauth2/authorize',
    revokeEndpoint: 'https://auth.hume.ai/oauth2/revoke',
    scopes: ['evi.tts', 'evi.prompts', 'evi.voices', 'management.read'],
    defaultRedirectUri: 'https://sallyport.2100.cool/oauth/hume/callback'
  }
};
...
class OAuth2Client {
  constructor(options) {
    // Auto-configure for AI services if provider matches
    const aiProvider = AI_PROVIDERS[options.provider];
    if (aiProvider) {
      console.log(chalk.blue(`[OAuth2] Auto-configuring for AI service: ${options.provider}`));
      this.tokenEndpoint = options.tokenEndpoint || aiProvider.tokenEndpoint;
      this.authEndpoint = options.authEndpoint || aiProvider.authEndpoint;
      this.revokeEndpoint = options.revokeEndpoint || aiProvider.revokeEndpoint;
      this.scopes = options.scopes || aiProvider.scopes;
      this.redirectUri = options.redirectUri || aiProvider.defaultRedirectUri;
    } else {
      this.tokenEndpoint = options.tokenEndpoint;
      this.authEndpoint = options.authEndpoint;
      this.revokeEndpoint = options.revokeEndpoint;
  ...
  /**
   * Factory method to create OAuth2 client for AI services
   * @param {string} service - AI service name (anthropic, openai, hume)
   * @param {Object} options - Additional options
   * @returns {OAuth2Client} Configured OAuth2 client
   */
  static createForAIService(service, options = {}) {
    const TokenStore = require('./token-store');
    const tokenStore = options.tokenStore || new TokenStore();
    
    const serviceUpper = service.toUpperCase();
    const companyName = options.companyName || 'shared';
    
    return new OAuth2Client({
      provider: service,
      clientId: process.env[`${serviceUpper}_OAUTH_CLIENT_ID`] || options.clientId,
      clientSecret: process.env[`${serviceUpper}_OAUTH_CLIENT_SECRET`] || options.clientSecret,
      redirectUri: process.env[`${serviceUpper}_OAUTH_REDIRECT_URI`] || options.redirectUri,
      tokenStore,
   ...
   * @returns {Array<string>} List of supported AI service names
   */
