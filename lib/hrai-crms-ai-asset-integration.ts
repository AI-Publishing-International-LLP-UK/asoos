...
export interface OwnerSubscriber {
  ...
  classification_type: 'MEMBER_LLP_CONTRACT' | 'MEMBER_LLP_EMPLOYEE' | 'NON_MEMBER_CONTRACT_EMPLOYEE' | 'MEMBER_LLP_NO_EMPLOYMENT';
...
export interface AIAgentAsset {
  ...
  agent_classification: 'RIX' | 'sRIX' | 'qRIX' | 'HQRIX' | 'MAESTRO-01' | 'MAESTRO-02' | 'MAESTRO-03' | 'CRX-01' | 'CRX-02' | 'PROFESSIONAL-COPILOT';
...
export class HRAICRMSAIAssetIntegration {
  ...
  async syncOwnerSubscriber(
    ...
    const ownerSubscriber: OwnerSubscriber = {
      ...
      employment_relationship: this.getEmploymentRelationship(ownerData.hr_classification || '.hr4'),
    ...
    await this.ownerSubscribers!.updateOne(
      { uuid },
      { $set: ownerSubscriber },
      { upsert: true }
    );
    ...
    console.log(`✅ Owner subscriber synced: ${ownerData.name} (${ownerSubscriber.hr_classification})`);
  ...
  async registerAIAgentAsset(
    ...
    console.log(`✅ AI agent registered as asset: ${pilotAgentName} - £${agentData.initial_cost.toLocaleString()}`);
    return agentAsset;
  }
  ...
  async recordVLSFlight(
    ...
    console.log(`✅ VLS flight recorded: ${flightRecord.flight_id} - Value enhancement: £${valueEnhancement.toFixed(2)}`);
  ...
  async generateBalanceSheetReport(reportDate: Date = new Date()): Promise<any> {
    ...
    const balanceSheetReport = {
      ...
      asset_summary: {
        ...
        average_book_value_per_agent: (aggregateResult?.total_ai_assets_book_value || 0) / (aggregateResult?.asset_count || 1),
  ...
  private calculateValueEnhancement(
    ...
    const enhancement = currentBookValue * baseEnhancementRate * (compositeScore / 100) * hoursFlown;
    
    // Cap enhancement at 1% of book value per flight for prudence
    return Math.min(enhancement, currentBookValue * 0.01);
  }

  private determineFlightOutcome(metrics: VLSFlightRecord['performance_metrics']): VLSFlightRecord['flight_outcome'] {
    const averageScore = (metrics.efficiency_score + metrics.accuracy_score + metrics.innovation_score + metrics.collaboration_score) / 4;
  ...
  private async updateAgentAssetFromFlight(
    ...
    const updatedAsset = {
      ...
      flight_performance_score: this.calculateRunningAverage(
        ...
        (flight.performance_metrics.efficiency_score + flight.performance_metrics.accuracy_score +
         flight.performance_metrics.innovation_score + flight.performance_metrics.collaboration_score) / 4
      ),
      'vls_tracking.last_flight_date': flight.flight_date,
      'vls_tracking.total_flight_hours': asset.vls_tracking.total_flight_hours + (flight.flight_duration_minutes / 60),
  ...
  private async calculateWeeklyDepreciation(asset: AIAgentAsset): Promise<void> {
    ...
    const depreciationEntry: UKGAAPDepreciationEntry = {
      ...
      useful_life_remaining_years: asset.useful_life_years - ((Date.now() - asset.acquisition_date.getTime()) / (365.25 * 24 * 60 * 60 * 1000)),
    ...
    try {
      ...
      switch (command) {
        case 'sync-owner':
          ...
          if (!uuid || !tenantId) {
            console.error('Usage: node hrai-crms-ai-asset-integration.js sync-owner <uuid> <tenantId>');
            process.exit(1);
          }
        ...
        default:
          ...
          console.log('Commands: sync-owner, register-agent, record-flight, weekly-depreciation, balance-sheet');
          break;
      }
    } catch (error) {
