...
class UniversalAIKeyManager {
  constructor(options = {}) {
    ...
    this.registryPath =
      options.registryPath || path.join(__dirname, '../Aixtiv-Symphony/mcp-company-registry.json');
  ...
  async getAPIKey(service, companyName = null, tier = 'managed-basic', options = {}) {
    ...
    if (!serviceConfig) {
      throw new Error(
        `Unsupported AI service: ${service}. Supported services: ${Object.keys(this.aiServices).join(', ')}`
      );
    }
  ...
  async getCustomerProvidedKey(service, companyName, tier) {
    ...
    try {
      ...
      console.error(
        `💡 Customer must provide their ${this.aiServices[service].name} API key via the onboarding process`
      );
      await this.logKeyUsage(service, companyName, tier, 'customer-provided', 'missing');
      throw new Error(
        `Customer must provide their own ${this.aiServices[service].name} API key for ${tier} tier`
      );
    }
  }
  ...
  async getDedicatedKey(service, companyName, tier, autoProvision = false) {
    ...
    try {
      ...
      if (autoProvision && this.aiServices[service].supportsProvisioning) {
        ...
        throw new Error(
          `Dedicated ${this.aiServices[service].name} API key not configured for ${companyName}`
        );
      }
    }
  }
  ...
  async getSharedKey(service, allowFallback = true) {
    ...
    try {
      ...
      if (allowFallback) {
        ...
        if (envKey) {
          console.log(
            `⚠️ Using environment variable fallback for ${this.aiServices[service].name}`
          );
  ...
  async validateAPIKey(service, apiKey, companyName = 'test') {
    ...
    try {
      ...
      console.log(
        `${isValid ? '✅' : '❌'} ${serviceConfig.name} API key validation: ${response.status}`
      );
...
if (require.main === module) {
  async function main() {
    ...
    if (!service) {
      ...
      console.log('\nSupported services:');
      keyManager.getSupportedServices().forEach((s) => console.log(`  ${s.key} - ${s.name}`));
      process.exit(1);
    }
