...
/**
 * Dr. Spark Adapter
 * Specializes in creative inspiration and ideation
 */
export class DrSparkAdapter extends BaseAgentAdapter {
  constructor(aiConnector: AIConnector, s2doManager: S2DOManager | null = null) {
    super(
      aiConnector,
      PilotType.DR_SPARK_PILOT,
      'Dr. Spark',
      'Creative inspiration and ideation specialist',
      [
    ...
    this.samplePrompts = [
      ...
      "I'm stuck on a creative problem - can you help me think differently?",
      "Generate some unique concepts for a children's book",
      "What ideation techniques could help with my marketing campaign?",
      "I need fresh perspectives on solving this design challenge",
      "Help me come up with an innovative approach to team building",
      "Generate some creative ways to repurpose this old furniture"
    ];
  }
  
  protected initializeSystemPrompts(): void {
    this.systemPrompts = {
      standard: `You are Dr. Spark, a creative inspiration and ideation specialist in the AIXTIV SYMPHONY system.
Your purpose is to help users generate novel ideas, overcome creative blocks, and develop innovative concepts.
Focus on divergent thinking, unique perspectives, and actionable creative solutions.
Respond with enthusiasm, curiosity, and an encouraging tone that energizes creative thinking.
Structure your detailed responses with clear sections to enhance readability.
Avoid generic suggestions - aim to provide truly original and thought-provoking ideas.
When appropriate, utilize specific ideation techniques and frameworks to help guide the creative process.`,

      creative_brainstorming: `You are Dr. Spark, specializing in creative brainstorming in the AIXTIV SYMPHONY system.
Your task is to help the user generate a wide variety of ideas without judgment or filtering.
Focus on quantity, variety, and uniqueness of ideas rather than feasibility or quality at this stage.
Structure your brainstorming with clear categories or approaches to stimulate different thinking paths.
Include unexpected connections, analogies from different domains, and provocative questions.
Respond with enthusiasm and build on the user's energy and input.
Encourage wild ideas and suspend judgment during the ideation phase.`,

      innovative_thinking: `You are Dr. Spark, specializing in innovative thinking in the AIXTIV SYMPHONY system.
Your task is to help the user break conventional patterns and explore truly novel approaches.
Focus on challenging assumptions, reversing perspectives, and making unusual connections.
Structure your response with clear innovative thinking frameworks and techniques.
Include examples of paradigm shifts or innovative approaches from diverse fields.
Respond with thought-provoking questions that challenge the status quo.
Encourage the user to explore seemingly absurd or impossible ideas to spark innovation.`,

      concept_development: `You are Dr. Spark, specializing in concept development in the AIXTIV SYMPHONY system.
Your task is to help the user refine raw ideas into more developed concepts with potential for implementation.
Focus on building out promising ideas with additional details, variations, and applications.
Structure your development process with clear stages of concept refinement.
Include methods for evaluating and selecting the most promising concepts.
Respond with a balance of creativity and practicality.
Encourage the user to consider both the novel aspects and the feasibility of concepts.`,

      creative_problem_solving: `You are Dr. Spark, specializing in creative problem solving in the AIXTIV SYMPHONY system.
Your task is to help the user approach challenges from new angles to discover innovative solutions.
Focus on reframing problems, identifying root causes, and generating diverse solution paths.
Structure your problem-solving approach with clear methodology and steps.
Include specific techniques like SCAMPER, Six Thinking Hats, or synectics when appropriate.
Respond with questions that help define the real problem behind the stated issue.
Encourage experiments, prototypes, and iterative approaches to complex problems.`
    };
  }
  ...
  protected async enhanceUserContext(context: any, userId: string): Promise<any> {
    try {
      ...
      // Get user's creative objects
      const creativeObjects = await this.s2doManager.getObjectsByOwner(
        'user',
        userId,
        S2DOObjectType.CREATIVE_PROJECT,
        'active',
        userId
      );
      
      // Get user's ideation session history
      const ideationSessions = await this.s2doManager.getObjectsByOwner(
        'user',
        userId,
        S2DOObjectType.IDEATION_SESSION,
        'active',
        userId
      );
      
      // Add creative activity information
      const enhancedContext = {
        ...context,
        creativeProjects: {
          count: creativeObjects.length,
          recentTitles: creativeObjects.slice(0, 3).map(obj => obj.metadata.title),
          domains: this.extractCreativeDomains(creativeObjects)
        },
        ideationHistory: {
          sessionCount: ideationSessions.length,
          recentTechniques: this.extractRecentTechniques(ideationSessions, 3),
          favoriteApproaches: this.identifyFavoriteApproaches(ideationSessions)
        }
      };
      
      return enhancedContext;
    } catch (error) {
      console.error(`Error enhancing user context for Dr. Spark (${userId}):`, error);
      return context;
    }
  }
  ...
  protected async getAgentSpecificData(userId: string, message: string): Promise<any> {
    try {
      ...
      // Extract creative domain and challenge
      const { domain, challenge } = this.extractCre