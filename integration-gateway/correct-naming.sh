...
echo "Correcting naming from 'salleyport' to 'sallyport' product-wide..."

# First, fix the remaining specific agents
cat > activate-corrected-agents.sh << 'EOFS'
#!/bin/bash
...
chmod +x activate-corrected-agents.sh
./activate-corrected-agents.sh

echo "Corrected activation of agents completed."

# Now let's search for any other instances of "salleyport" in the codebase
echo "Searching for other instances of 'salleyport' in the codebase..."
grep -r "salleyport" --include="*.js" --include="*.sh" --include="*.md" .

echo "Note: To fully correct the naming across the entire product, you'll need to:"
echo "1. Update all scripts and code files that contain 'salleyport'"
echo "2. Update any documentation or README files"
echo "3. Update any UI text or display strings"
echo "4. Update database records if appropriate"

echo "Correction process completed. Please run 'aixtiv claude:status' to verify agents are active."
