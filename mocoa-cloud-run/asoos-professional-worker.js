  ...
  <script>
    ...
    function createParticles() {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.left = Math.random() * PERCENTAGE_MULTIPLIER + '%';
        particle.style.top = Math.random() * PERCENTAGE_MULTIPLIER + '%';
    ...
    function animateNumber(element, target, suffix) {
      ...
      const timer = setInterval(() => {
        ...
        if (suffix === 'M+') {
          ...
          displayValue = (current / MS_TO_SECONDS).toFixed(0) + suffix;
        } else {
...
// OAuth2 Configuration - kept for future implementation
...
async function handleOAuth2Callback(url) {
  const code = url.searchParams.get('code');

  if (!code) {
    return Response.redirect('https://asoos.2100.cool/?error=missing_code', REDIRECT_STATUS);
  }

  // TODO: Exchange code for tokens with OAuth2 provider
  return Response.redirect('https://2100.cool/interface?auth=success', REDIRECT_STATUS);
}
...
function generateToken(email, user) {
  const payload = {
    ...
    level: user.level,
    timestamp: Date.now(),
    expires: Date.now() + TOKEN_EXPIRY_MINUTES * MINUTES_TO_MS * MS_TO_S, // 30 minutes
  };
