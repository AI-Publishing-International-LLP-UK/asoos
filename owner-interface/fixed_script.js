import { rixVoiceConfig, elevenLabsVoices, drLucyServiceAccount, victory36ServiceAccount, drClaudeServiceAccount } from './js/agents.js';
import { getToken, isAuthenticated, enforceAuthentication, performLogin } from './js/auth.js';
...
async function initializeOAuth2() {
  ...
  // This is a temporary measure. In a real app, you'd have a login page.
  if (!isAuthenticated()) {
    console.log('No token found, performing mock login...');
    performLogin();
  }

  // Enforce that a user must be authenticated to use the app.
  enforceAuthentication();

  // Set the global token variable from the centralized auth module.
  oauthToken = getToken();
  apiConnected = !!oauthToken;
...
async function getSecretValue(secretName) {
  try {
    ...
    const result = await apiService.get(`/gcp/secrets/${secretName}`, {
      headers: {
    ...
    if (window.productionLogging) {
      window.productionLogging.info(`Secret ${secretName} retrieved successfully.`);
    }
...
function initializeDrLucyVoice() {
  if ('speechSynthesis' in window) {
    ...
    function selectDrLucyVoice() {
      ...
      // Primary: Look for 4RZ84U1b4WCqpu57LvIq (case insensitive)
      drLucyVoice = voices.find(voice =>
        voice.name.toLowerCase().includes('dana') ||
        voice.name.toLowerCase().includes('dana (premium)') ||
        voice.name.toLowerCase().includes('4RZ84U1b4WCqpu57LvIq') ||
        voice.name === 'Dana'
      );

      if (drLucyVoice) {
        console.log('✅ Found 4RZ84U1b4WCqpu57LvIq:', drLucyVoice.name);
        showNotification(`🎙️ Dr. Lucy ready with 4RZ84U1b4WCqpu57LvIq: ${drLucyVoice.name}`, 'success');
        return true;
      }
...
// Dr. Lucy speaks a message using OpenAI 4RZ84U1b4WCqpu57LvIq (primary)
function speakMessage(text) {
  console.log('🗣️ speakMessage called with:', text, 'using OpenAI 4RZ84U1b4WCqpu57LvIq');
  ...
  // Use OpenAI 4RZ84U1b4WCqpu57LvIq (primary for Dr. Lucy)
  isSpeaking = true;
  ...
  fetch('https://voice.asoos2100.cool/api/synthesize', {
    ...
    .then(audioBlob => {
      ...
      audio.onloadstart = () => {
        ...
        showNotification('🎙️ Dr. Lucy speaking with 4RZ84U1b4WCqpu57LvIq...', 'success');
      };

      audio.onplay = () => {
        console.log('✅ OpenAI 4RZ84U1b4WCqpu57LvIq playing');
      };

      audio.onended = () => {
        console.log('✅ OpenAI 4RZ84U1b4WCqpu57LvIq finished');
        isSpeaking = false;
      ...
      audio.onerror = (error) => {
        console.error('❌ OpenAI 4RZ84U1b4WCqpu57LvIq playback error:', error);
        isSpeaking = false;
        URL.revokeObjectURL(audioUrl);
...
async function speakMessage11LabsFallback(text) {
  ...
  fetch('https://api.elevenlabs.io/v1/text-to-speech/4RZ84U1b4WCqpu57LvIq', {
    method: 'POST',
    headers: {
      'Accept': 'audio/mpeg',
      'Content-Type': 'application/json',
...
function speakMessageOpenAIFallback(text) {
  ...
  // Call OpenAI TTS API with 4RZ84U1b4WCqpu57LvIq as fallback
  fetch('/api/speech/tts', {
    ...
    .then(audioBlob => {
      ...
      audio.onloadstart = () => {
        console.log('🎙️ OpenAI 4RZ84U1b4WCqpu57LvIq loading...');
...
// Stop current speech (for OpenAI TTS 4RZ84U1b4WCqpu57LvIq)
