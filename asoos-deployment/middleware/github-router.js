...
function isOrganizationAuthorized(organization, owner) {
  const orgSettings =
    organizationConfig.organizations[organization] || organizationConfig.organizations.default;
  if (!orgSettings) {
    return false;
  }
...
const rateLimitByAgent = (req, res, next) => {
  ...
  const orgSettings =
    organizationConfig.organizations[organization] || organizationConfig.organizations.default;
  const limit =
    orgSettings.rateLimits[agentType] || organizationConfig.globalSettings.defaultRateLimit;
