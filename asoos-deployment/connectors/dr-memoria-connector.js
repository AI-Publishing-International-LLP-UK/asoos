...
class DrMemoriaLinkedInConnector {
  ...
  async processOrganizations(organizations) {
    ...
    try {
      ...
      console.log(
        `✅ Dr. Memoria completed processing: ${results.filter((r) => !r.error).length}/${organizations.length} successful`
      );
  ...
  async lookupByLinkedInHandle(handle) {
    try {
      const response = await this.makeLinkedInAPICall(
        `/v2/companies/(universalName:${handle})`,
        'GET'
      );
  ...
  async checkRateLimit() {
    ...
    if (this.rateLimits.currentUsage >= this.rateLimits.companiesPerHour) {
      ...
      console.log(
        `⏳ Dr. Memoria rate limit reached, waiting ${Math.ceil(waitTime / 60000)} minutes...`
      );
