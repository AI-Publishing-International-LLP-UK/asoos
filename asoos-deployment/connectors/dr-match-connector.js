...
class DrMatchLinkedInConnector {
  ...
  async processOrganizations(organizations) {
    console.log(
      `🎯 Dr. Match processing ${organizations.length} organizations for talent insights...`
    );
    ...
    try {
      for (let i = 0; i < organizations.length; i += batchSize) {
        ...
        console.log(
          `📈 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(organizations.length / batchSize)}`
        );
      ...
      console.log(
        `✅ Dr. Match completed processing: ${results.filter((r) => !r.error).length}/${organizations.length} successful`
      );
  ...
  async getCompanyData(organization) {
    try {
      ...
      if (organization.linkedinHandle) {
        const response = await this.makeLinkedInAPICall(
          `/v2/companies/${organization.linkedinHandle}`,
          'GET'
        );
        if (response.data) {
      ...
      if (
        searchResponse.data &&
        searchResponse.data.elements &&
        searchResponse.data.elements.length > 0
      ) {
  ...
  async getLeadershipData(organization, companyData) {
    try {
      ...
      for (const role of leadershipRoles.slice(0, 3)) {
        // Limit to avoid rate limits
        const leaders = await this.searchPeopleByRole(
          companyData.linkedinId || companyData.universalName,
          role
        );
  ...
  async checkRateLimit() {
    ...
    if (this.rateLimits.currentUsage >= this.rateLimits.companiesPerHour) {
      ...
      console.log(
        `⏳ Dr. Match rate limit reached, waiting ${Math.ceil(waitTime / 60000)} minutes...`
      );
