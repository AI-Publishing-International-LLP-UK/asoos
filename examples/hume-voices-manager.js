    ...
    try {
      ...
      return {
        ...
        total: {
          ...
          total_count:
            (presetVoices.voices_page?.length || 0) + (customVoices.voices_page?.length || 0),
        },
      };
    } catch (error) {
  ...
  try {
    ...
    if (!apiKey) {
      ...
      try {
        ...
        apiKey = execSync('gcloud secrets versions access latest --secret="hume-api-key"', {
          encoding: 'utf8',
        }).trim();
        ...
        console.error(
          'Please ensure you have access to the hume-api-key secret or set HUME_API_KEY environment variable'
        );
        process.exit(1);
      }
    }
