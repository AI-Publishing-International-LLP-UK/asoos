...
class OAuth2SallyPortTester {
  constructor() {
    this.baseUrl = 'https://integration-gateway-yutylytffa-uw.a.run.app';
    this.results = {
  ...
  validateJWTClaims(payload, tenant) {
    ...
    if (tenant.name === 'zaxon') {
      if (payload.company !== tenant.expected_company) {
        errors.push(`Company mismatch: expected ${tenant.expected_company}, got ${payload.company}`);
      }
      if (payload.sao_level !== tenant.expected_sao_level) {
        errors.push(`SAO level mismatch: expected ${tenant.expected_sao_level}, got ${payload.sao_level}`);
      }
