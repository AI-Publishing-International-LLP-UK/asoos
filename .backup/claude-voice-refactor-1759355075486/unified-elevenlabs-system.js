...
class UnifiedElevenLabsAgentSystem {
  constructor(options = {}) {
    ...
    this.oauth2Config = {
      authorizationUrl:
        ...
        'https://auth.api-for-warp-drive.com/oauth2/authorize',
      tokenUrl: process.env.OAUTH2_TOKEN_URL || 'https://auth.api-for-warp-drive.com/oauth2/token',
      clientId: process.env.OAUTH2_CLIENT_ID,
      clientSecret: process.env.OAUTH2_CLIENT_SECRET,
      redirectUri:
        process.env.OAUTH2_REDIRECT_URI || 'https://mocoa.api-for-warp-drive.com/auth/callback',
      scopes: [
        'elevenlabs:tts',
  ...
  async loadCredentials() {
    try {
      const secrets = ['OAUTH2_CLIENT_ID', 'OAUTH2_CLIENT_SECRET', 'JWT_SECRET'];
      ...
      for (const secretName of secrets) {
        try {
          ...
          if (
            secretValue &&
            secretValue.trim() !== '' &&
            secretValue !== 'your_elevenlabs_api_key_here'
          ) {
          ...
          this.logger.warn(
            `⚠️ Could not load ${secretName} from Secret Manager, using environment variable`
          );
        }
      }
      ...
      if (!this.oauth2ClientId || !this.oauth2ClientSecret) {
        this.logger.warn(
          '⚠️ OAuth2 credentials not found, initializing self-healing OAuth2 system...'
        );
      ...
      this.logger.info(
        '✅ OAuth2-only authentication mode enabled - API key authentication disabled'
      );
    } catch (error) {
  ...
  setupRoutes() {
    ...
    this.app.post(
      '/api/protected/owner-interface/voice',
      this.generateOwnerInterfaceVoice.bind(this)
    );
